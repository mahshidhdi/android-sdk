...
class SessionFlowManagerTest {
    ...
    private val pusheConfig: PusheConfig = mockk(relaxed = true)
    private val moshi = PusheMoshi()
    private val storageFactory = PusheStorage(moshi, sharedPreferences)
    ...
    fun setUp() {
        ...
        co.pushe.plus.analytics.extendMoshi(moshi)

        sessionFlowManager = SessionFlowManager(currentTimeGenerator, postOffice, pusheConfig, mockk(relaxed = true), storageFactory)
        every { pusheConfig.sessionFragmentFlowDepthLimit } returns 5
        every { pusheConfig.sessionFragmentFlowEnabled } returns true
        every { pusheConfig.sessionFragmentFlowExceptionList } returns emptyList()
    }
    ...
    fun updateSessionFlow_checksForTheFragmentToBeEnabled_addsParentsIfTheyAreEnabled() {
        ...
        every { pusheConfig.sessionFragmentFlowEnabled } returns false
        ...
        setSessionFlow(sampleSessionFlowWithoutFragment)
        every { pusheConfig.sessionFragmentFlowEnabled } returns false
        every { pusheConfig.sessionFragmentFlowExceptionList } returns listOf(FragmentFlowInfo("SecondActivity", "id12"))
        ...
        setSessionFlow(sampleSessionFlowWithoutFragment)
        every { pusheConfig.sessionFragmentFlowEnabled } returns true
        every { pusheConfig.sessionFragmentFlowDepthLimit } returns 2
        every { pusheConfig.sessionFragmentFlowExceptionList } returns listOf(FragmentFlowInfo("SecondActivity", "id12"))
