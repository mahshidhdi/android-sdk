...
class PostOffice @Inject constructor(
     ...
     *                          next run
     * @param requiresRegistration If this parameter is true the message will only be sent if Pushe
    ...
    private fun isFullParcelReady(): Single<Boolean> {
        ...
        val maxParcelSize = pusheConfig.upstreamMaxParcelSize
     ...
     *
     * The sum of all message sizes in a parcel will not exceed [PusheConfig.upstreamMaxParcelSize],
     ...
     * the time specified by [PusheConfig.upstreamMessageTimeout] and we have not received an ACK or error
     * for it.
     *
    ...
    fun checkInFlightMessageTimeouts(): Completable {
        ...
        val messageTimeout = pusheConfig.upstreamMessageTimeout
        val newState = UpstreamMessageState.Stored()
        return messageStore.readMessages()
                .filter { it.messageState is UpstreamMessageState.InFlight }
     ...
     * [PusheConfig.upstreamMessageExpirationTime] value. Once this amount of time has passed since
    ...
    fun checkMessageExpirations(): Completable {
        val now = TimeUtils.now()
        val defaultExpirationTime = pusheConfig.upstreamMessageExpirationTime
        return messageStore.readMessages()
                .filter { it.messageState is UpstreamMessageState.Stored }
