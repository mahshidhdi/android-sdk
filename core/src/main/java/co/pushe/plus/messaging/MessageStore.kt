...
import co.pushe.plus.maxPendingUpstreamMessagesForType
...
class MessageStore @Inject constructor(
     ...
     * [PusheConfig.maxPendingUpstreamMessagesPerType]. A pending message is any message which
    ...
    fun storeMessage(message: UpstreamMessage, sendPriority: SendPriority,
        ...
        if ((messageCountPerType[message.messageType] ?: 0) >= pusheConfig.maxPendingUpstreamMessagesForType(message.messageType)) {
    ...
    companion object {
        const val MESSAGE_STORE_NAME = "pushe_message_store"
