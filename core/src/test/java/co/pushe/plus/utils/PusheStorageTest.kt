...
import co.pushe.plus.utils.test.TestUtils.advanceMockTimeBy
...
class PusheStorageTest {
    ...
    fun storedMap_PersistsUpdatesInStorage() {
        val storageFactory = PusheStorage(PusheMoshi(), MockSharedPreference())
    ...
    fun storedString_PersistsAndRestoresAndDeletesValue() {
        ...
        var storedItem by PusheStorage(PusheMoshi(), sharedPreference).storedString("key1", "default")
        assertEquals("default", storedItem)
        storedItem = "value"
        assertEquals("value", storedItem)

        advanceTimeBy(seconds(2))
        val restoredItem = PusheStorage(PusheMoshi(), sharedPreference).storedString("key1", "default")
        assertEquals("value", restoredItem.get())
        restoredItem.delete()
        assertEquals("default", restoredItem.get())

        advanceTimeBy(seconds(2))
        val restoredItem2 by PusheStorage(PusheMoshi(), sharedPreference).storedString("key1", "default")
    ...
    fun storedInt_PersistsAndRestoresAndDeletesValue() {
        val sharedPreference = MockSharedPreference()
        var storedItem by PusheStorage(PusheMoshi(), sharedPreference).storedInt("key1", 0)
        assertEquals(0, storedItem)
        storedItem = 1000
        assertEquals(1000, storedItem)

        advanceTimeBy(seconds(2))
        val restoredItem = PusheStorage(PusheMoshi(), sharedPreference).storedInt("key1", 0)
        assertEquals(1000, restoredItem.get())
        restoredItem.delete()
        assertEquals(0, restoredItem.get())

        advanceTimeBy(seconds(2))
        val restoredItem2 by PusheStorage(PusheMoshi(), sharedPreference).storedInt("key1", 0)
    ...
    fun storedLong_PersistsAndRestoresAndDeletesValue() {
        val sharedPreference = MockSharedPreference()
        var storedItem by PusheStorage(PusheMoshi(), sharedPreference).storedLong("key1", 0L)
        assertEquals(0L, storedItem)
        storedItem = 1000L
        assertEquals(1000L, storedItem)

        advanceTimeBy(seconds(2))
        val restoredItem = PusheStorage(PusheMoshi(), sharedPreference).storedLong("key1", 0L)
        assertEquals(1000L, restoredItem.get())
        restoredItem.delete()
        assertEquals(0L, restoredItem.get())

        advanceTimeBy(seconds(2))
        val restoredItem2 by PusheStorage(PusheMoshi(), sharedPreference).storedLong("key1", 0L)
    ...
    fun storedFloat_PersistsAndRestoresAndDeletesValue() {
        val sharedPreference = MockSharedPreference()
        var storedItem by PusheStorage(PusheMoshi(), sharedPreference).storedFloat("key1", 0f)
        assertEquals(0f, storedItem)
        storedItem = 1000f
        assertEquals(1000f, storedItem)

        advanceTimeBy(seconds(2))
        val restoredItem = PusheStorage(PusheMoshi(), sharedPreference).storedFloat("key1", 0f)
        assertEquals(1000f, restoredItem.get())
        restoredItem.delete()
        assertEquals(0f, restoredItem.get())

        advanceTimeBy(seconds(2))
        val restoredItem2 by PusheStorage(PusheMoshi(), sharedPreference).storedFloat("key1", 0f)
    ...
    fun storedBoolean_PersistsAndRestoresAndDeletesValue() {
        val sharedPreference = MockSharedPreference()
        var storedItem by PusheStorage(PusheMoshi(), sharedPreference).storedBoolean("key1", false)
        assertEquals(false, storedItem)
        storedItem = true
        assertEquals(true, storedItem)

        advanceTimeBy(seconds(2))
        val restoredItem = PusheStorage(PusheMoshi(), sharedPreference).storedBoolean("key1", false)
        assertEquals(true, restoredItem.get())
        restoredItem.delete()
        assertEquals(false, restoredItem.get())

        advanceTimeBy(seconds(2))
        val restoredItem2 by PusheStorage(PusheMoshi(), sharedPreference).storedBoolean("key1", false)
    ...
    fun rateLimitsPersistingOnFrequentUpdates() {
        ...
        val storageFactory = PusheStorage(PusheMoshi(), sharedPreferences)
        val storedMap = storageFactory.createStoredMap("test_map", Int::class.javaObjectType)
