...
import co.pushe.plus.notification.utils.MaterialIconHelper
import co.pushe.plus.notification.utils.getNotificationButtonIds
...
class NotificationBuilder constructor(
     ...
     * `stepFunc`.
     *
     * If executing the step takes longer than the timeout defined by [PusheConfig.notificationBuildStepTimeout]
    ...
    private fun <T> performStep(buildStep: NotificationBuildStep, stepFunc: Single<T>, doIfLimitReached: Single<T>? = null): Single<T>? {
        return try {
            ...
            val timeout = pusheConfig.notificationBuildStepTimeout(buildStep)
            buildStepFunction
    ...
    private fun setNotificationAutoCancel(builder: Notification.Builder) {
        /*
         * Hadi: In the legacy Pushe SDK cancelling was done in the service run by the pending intent.
    ...
    private fun setNotificationBackgroundImage(builder: Notification.Builder) {
        if (!message.justImgUrl.isNullOrBlank()) {
            val contentView = RemoteViews(context.packageName, R.layout.pushe_custom_notification)
            val backgroundImg = imageDownloader.getImage(message.justImgUrl)
            contentView.setImageViewBitmap(R.id.pushe_notif_bkgrnd_image, backgroundImg)
            when {
    ...
    private fun setNotificationBigContent(builder: Notification.Builder) {
        when {
            ...
            else -> Plog.warn(T_NOTIF, "Pushe", "Notification style not supported below Android 4.1")
        }
    }
        ...
        if (message.usePusheIcon) {
            ...
            if (SDK_INT >= M && message.smallIconUrl != null && isValidWebUrl(message.smallIconUrl)) {
                ...
                if (resId > 0) { // if user specified an icon from drawable as its smallIcon
                    ...
                    val emptyIconId = context.resources.getIdentifier("pushe_ic_empty", "drawable", context.packageName)
                    builder.setSmallIcon(
    ...
    private fun setNotificationButtons(builder: Notification.Builder) {
        when {
            SDK_INT >= JELLY_BEAN -> {
                ...
                message.buttons.forEachIndexed { index, button ->
                    val iconId = if (button.icon.isNullOrBlank()) {
                        context.resources.getIdentifier("pushe_ic_empty", "drawable", context.packageName)
                    } else {
            ...
            else -> Log.w("Pushe", "Notification buttons not supported below Android 4.1")
        }
    }
    ...
    /**
     * Migrated from Pushe v1.4.1 `NotificationController.changeIcon`
     */
        ...
        return Single.defer {
            ...
            if (ringerMode == AudioManager.RINGER_MODE_VIBRATE) {
                ...
                if (message.soundUrl != null && isValidWebUrl(message.soundUrl) && notificationSettings.isCustomSoundEnabled) {
                    ...
                    notification.defaults = notification.defaults or Notification.DEFAULT_VIBRATE
                    return@defer NotificationSoundPlayer(message.soundUrl, pusheConfig.notificationMaxSoundDuration)
                            .play()
...
class NotificationBuilderFactory @Inject constructor (
    ...
    fun createNotificationBuilder(message: NotificationMessage): NotificationBuilder {
        return NotificationBuilder(message, context, notificationSettings, errorHandler, imageDownloader, pusheConfig, moshi)
    }
}