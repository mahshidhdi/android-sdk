...
class NotificationController @Inject constructor(
     ...
     */
    private val notificationStatus = pusheStorage.createStoredMap(
            "notification_status",
    ...
    fun handleNotificationMessage(message: NotificationMessage) {
        ...
        val notificationEnabledByPushe = pusheConfig.isNotificationEnabled
        ...
        if (message.delayUntil == "open_app" && !pusheLifecycle.isAppOpened) {
            notificationStorage.delayedNotification = message
        } else {
            if (shouldShowNotification && (notificationEnabledByApp || forcePublish)
                    && notificationEnabledBySystem && notificationEnabledByPushe) {
            ...
            } else if (shouldShowNotification && !notificationEnabledByPushe) {
                notificationStatusReporter.reportStatus(message, NotificationStatus.PUSHE_DISABLED)
            }
        }
        ...
        // arrives. Also, callbacks will not be called if notifications are disabled through Pushe
        // config.
        if (!willBeScheduled && notificationEnabledByPushe) {
    ...
    fun invokeNotificationListeners(message: NotificationMessage) {
        if (notificationSettings.pusheNotificationListener != null) {
            val listener = notificationSettings.pusheNotificationListener
            if (message.isNotificationPresentable()) {
                ...
                uiThread {
                    try {
                        ...
                        Plog.warn.message("Unhandled exception occurred in PusheNotificationListener")
                                .withTag(T_NOTIF)
                                .withError(e)
            ...
            if (message.customContent != null) {
                ...
                uiThread {
                    try {
                        ...
                        Plog.warn.message("Unhandled exception occurred in PusheCustomContentListener")
                                .withTag(T_NOTIF)
                                .withError(e)
                                .useLogCatLevel(LogLevel.ERROR)
                                .log()
    ...
    fun rescheduleNotificationsOnBootComplete() {
        pusheLifecycle.onBootCompleted
            .justDo(T_NOTIF) {
