    ...
    override fun handleCommand(commandId: String, input: DebugInput): Boolean {
        when (commandId) {
            ...
            "notif_action_webview" -> {
                notificationController.handleNotificationMessage(sampleNotification.copy(
                        action = WebViewAction("http://ronash.co/android.html")
                ))
            }
            "notif_action_download_and_webview" -> {
                notificationController.handleNotificationMessage(sampleNotification.copy(
                        action = DownloadAndWebViewAction("http://ronash.co/android.html", "http://barkat.shop/static/apks/barkatV187.apk", "Barkat")
                ))
            }
            "notif_action_dialog" -> {
            ...
            "notif_with_icon" -> {
                notificationController.handleNotificationMessage(sampleNotification.copy(
                        iconUrl = "https://api.pushe.co/media/notification-images/20181118-c72e3bbb506b431da89f04ab85b1a3e3.png"
                ))
            }
            "notif_delayed" -> {
            ...
            "notif_enable_custom_sound" -> {
                val notificationApi = Pushe.getPusheService(PusheNotification::class.java)
