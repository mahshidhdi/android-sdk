...
import co.pushe.plus.admin.analytics.activities.*
...
import co.pushe.plus.analytics.goal.*
...
class AppLifeCycleListenerTest {
    ...
    private val pusheLifecycle: PusheLifecycle = PusheLifecycle(context)
    ...
    private val storage = PusheStorage(moshi, sharedPreference)
    ...
    private val frameLayoutActivityFragmentWithLayoutsInfo =
            SessionFragmentInfo(
                    "co.pushe.plus.admin.analytics.fragments.FragmentWithLayouts",
                    "flContainer",
                    "MultipleFrameLayoutActivity"
            )
    ...
    fun init() {
        ...
        sessionFlowManager = spyk(SessionFlowManager(currentTimeGenerator, postOffice, pusheConfig, mockk(relaxed = true), storage))
...
//        every { pusheConfig.getLong(Config.SESSION_PAUSE_LIMIT, any()) } returns 20000L
//
